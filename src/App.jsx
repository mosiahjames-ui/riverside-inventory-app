  ...
  const inventoryRows = useMemo(() => {
    ...
    return books.map((book) => {
      ...
      return {
        ...
        status: stock.needs_reorder ? 'Reordered' : Number(stock.qty_in_stock ?? 0) <= Number(stock.low_stock_threshold ?? 0) ? 'Low Stock' : 'In Stock',
      }
    }).filter((row) => (!showLowStock || row.low) && (!searchQuery || `${row.title} ${books.find((book) => book.book_id === row.book_id)?.author ?? ''}`.toLowerCase().includes(searchQuery.toLowerCase())) && (genreFilter === 'all' || books.find((book) => book.book_id === row.book_id)?.genre === genreFilter))
  }, [books, inventory, showLowStock, searchQuery, genreFilter])
